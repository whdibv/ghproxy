...
- ⚡ 本地文件缓存，二次访问秒开
- 📦 流式下载，不占内存
- 🗜 **大文件流式转发**：50MB - 200MB 不落盘缓存，边下边传（避免撑爆磁盘）
- 🌍 **超大文件第三方分流**：超过 200MB 自动展示第三方高速节点选择页，不占用你的服务器带宽（节点在 `nodes.php` 中配置）
...
**Q：大文件（如 10GB）会怎样？**
A：分三档处理——
- **50MB 以内**：缓存到本地，二次访问秒回
- **50MB - 200MB**：流式转发（边下边传），不占磁盘，响应头 `X-Cache-Status: BYPASS`
- **超过 200MB**：自动展示**第三方高速节点选择页**（`X-Cache-Status: BIG-REDIRECT`），提供 Cloudflare 优选 / Fastly / AxisNow 等公共 CDN 节点一键下载——因为虚拟主机带宽有限，超大文件交给 CDN 分发速度更快，也不占你的带宽

**Q：200MB 以上为什么不让本机直接下载？**
A：虚拟主机通常只有 10Mbps 左右带宽，200MB 文件要传 20 多分钟，还容易触发主机限流。可配置第三方节点（公共 CDN 服务）分发，速度和稳定性远超个人虚拟主机。若某节点慢可切换其他节点。

**Q：第三方节点怎么配置？**
A：复制 `nodes.example.php` 为 `nodes.php`，填入你自己的节点（名称 + 代理前缀 + 说明）：

```php
<?php
return [
    ['name' => '节点名称', 'host' => 'https://你的代理前缀', 'note' => '说明文字'],
];
```

节点链接 = `host` + `/` + 原始 GitHub 链接。`nodes.php` 已在 `.gitignore` 中，不会被提交，自用节点安全。不配置时超大文件自动回退为流式转发。
